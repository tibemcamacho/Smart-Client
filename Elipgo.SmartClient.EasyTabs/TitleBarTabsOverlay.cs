...
namespace Elipgo.SmartClient.EasyTabs
{
	...
	public class TitleBarTabsOverlay : Form
	{
		...
		/// <summary>
		/// Ejecuta una acción en el hilo de UI de forma segura, verificando si el control está disposed.
		/// </summary>
		private void SafeInvoke(Action action)
		{
			try
			{
				if (this.IsDisposed || this.Disposing || _parentFormClosing)
					return;
		...
		private void _parentForm_FormClosing(object sender, CancelEventArgs e)
		{
			...
			// Detener el thread de eventos de mouse de forma segura
			_mouseEvents.CompleteAdding();
			// Esperar hasta 500ms para que el thread termine normalmente antes de abortarlo
			if (_mouseEventsThread != null && _mouseEventsThread.IsAlive)
			{
				if (!_mouseEventsThread.Join(500))
				{
					try { _mouseEventsThread.Abort(); } catch { }
				}
			}
		}
									...
									// If the cursor is outside the tear area, tear it away from the current window
										if (!dragArea.Contains(cursorPosition) && _tornTab == null)
										{
											...
											if (_tornTab != null)
											{
												...
												_parentForm.RemoveFormMainView(_tornTab); // Unregister detached tab from parent's "Forms"
			...
			switch ((WM) m.Msg)
			{
				...
				case WM.WM_LBUTTONDOWN:
					...
					else
					{
                        ...
                        bool isCloseButton = _parentForm.TabRenderer.IsOverCloseButton(clickedTab, relativeCursorPosition);
                        bool isDifferentTab = _parentForm.SelectedTab != clickedTab;
                        bool tabChanged = false;

                        // Tab change logic (Only if the user didn't click the close button and it's a different tab)
						if (!isCloseButton && isDifferentTab)
                        {
                            tabChanged = true;
                        ...
                        // If it wasn't a tab change (or it's the same tab), allow OnMouseDown to execute 
                        // so the renderer can establish the drag start point for a potential drag operation.
                        if (!tabChanged)
                        {
