...
namespace Elipgo.SmartClient.EasyTabs
{
    ...
    public abstract class BaseTabRenderer
    {
        ...
        protected Rectangle AreaAddButton { get; private set; }
        ...
        protected BaseTabRenderer(TitleBarTabs parentWindow)
        {
            ...
            main = Screen.PrimaryScreen.Bounds;
        }
        ...
        {
            if (AreaAddButton.Contains(Cursor.Position))
            {
                _parentWindow.AddNewTab();
            }
        }
        ...
        protected internal virtual void Overlay_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            // Check if the mouse has moved beyond the minimum drag threshold
            int horizontalDelta = Math.Abs(e.X - _dragStart.Value.X);
            int verticalDelta = Math.Abs(e.Y - _dragStart.Value.Y);

            if (horizontalDelta > TabRepositionDragDistance || verticalDelta > TabRepositionDragDistance)
            {
        ...
        protected bool IsOverNonTransparentArea(Rectangle area, Bitmap image, Point cursor)
        {
            ...
            return cursor.X >= area.X && cursor.X <= area.X + area.Width;
        }
        ...
        public virtual void Render(List<TitleBarTab> tabs, Graphics graphicsContext, Point offset, Point cursor, bool forceRedraw = false)
        {
            ...
            int tabContentWidth = Math.Min(_activeCenterImage.Width, Convert.ToInt32(Math.Floor(Convert.ToDouble(_maxTabArea.Width / tabs.Count)))) + (main.Width >= 2000 ? 70 : 0);
            ...
            var toolbarWidth = _parentWindow.IsVisibleToolBarIcon() ? 40 : 0;
            ...
            if (selectedIndex != -1)
            {
                ...
                var userIsLogin = _parentWindow.IsUserLogin(selectedTab) ? 0 : -20;

                Rectangle tabArea = new Rectangle(
                    SystemInformation.BorderSize.Width + offset.X +
                    selectedIndex * (tabContentWidth + tabLeftImage.Width + tabRightImage.Width - OverlapWidth) + toolbarWidth + 20 + userIsLogin,
            ...
            foreach (TitleBarTab tab in ((IEnumerable<TitleBarTab>)tabs).Reverse())
            {
                ...
                Rectangle tabArea =
                    new Rectangle(
                        SystemInformation.BorderSize.Width + offset.X +
                        (i * (tabContentWidth + tabLeftImage.Width + tabRightImage.Width - OverlapWidth)) + toolbarWidth + 20,
                        offset.Y + (TabHeight - tabCenterImage.Height), tabContentWidth + tabLeftImage.Width + tabRightImage.Width,
                        tabCenterImage.Height);
            ...
            if (ShowAddButton && !IsTabRepositioning)
            {
                _addButtonArea =
                    new Rectangle(
                        ...
                        _activeRightSideImage.Width + AddButtonMarginLeft + offset.X + toolbarWidth + 20,
                ...
                AreaAddButton = new Rectangle
                {
                    X = _addButtonArea.X,
                    Y = _addButtonArea.Y,
                    Width = _addButtonArea.Width + 5,
                    Height = _addButtonArea.Height + 5
                };
            }
        }
        ...
        protected virtual void Render(Graphics graphicsContext, TitleBarTab tab, int index, Rectangle area, Point cursor, Image tabLeftImage, Image tabCenterImage, Image tabRightImage)
        {
            ...
            if (tab.TabImage == null)
            {
                ...
                using (Graphics tabGraphicsContext = Graphics.FromImage(tab.TabImage))
                {
                    ...
                    tabGraphicsContext.DrawImage(
                        tabCenterImage, new Rectangle(tabLeftImage.Width, 0, _tabContentWidth + (main.Width >= 2000 ? 100 : 0), tabCenterImage.Height), 0, 0, _tabContentWidth, tabCenterImage.Height,
                        GraphicsUnit.Pixel);
            ...
            int sizeIco = (main.Width >= 2000 ? 25 : 16);
            area.Height = area.Height + (main.Width >= 2000 ? 9 : 0);
                ...
                : 0))
            {
                graphicsContext.DrawIcon(
                    new Icon(tab.Content.Icon, sizeIco, sizeIco),
                    new Rectangle(area.X + OverlapWidth + IconMarginLeft, IconMarginTop + area.Y, sizeIco, sizeIco));
            }

            var captionFont4k = new Font("Roboto", 26, FontStyle.Regular, GraphicsUnit.Pixel);
                    ...
                    : 0))
            {
                graphicsContext.DrawString(
                    tab.Caption, (main.Width >= 2000 ? captionFont4k : CaptionFont), new SolidBrush(ForeColor),
                    new Rectangle(
                        ...
                        _tabContentWidth - (tab.Content.ShowIcon
                            ? IconMarginLeft + sizeIco + IconMarginRight
