using DynamicData;
using Elipgo.SmartClient.Common;
using Elipgo.SmartClient.Common.DTOs;
using Elipgo.SmartClient.Common.Enum;
using Elipgo.SmartClient.Common.Helpers;
using Elipgo.SmartClient.Common.Properties;
using Elipgo.SmartClient.Services;
using Elipgo.SmartClient.Services.Services.Implement;
using Elipgo.SmartClient.Services.Services.Interface;
using ReactiveUI;
using Splat;
using System;
using System.Collections.Generic;
using System.Configuration;
using System.Linq;
using System.Threading.Tasks;

namespace Elipgo.SmartClient.ViewModels
{
    public delegate void OnNewAlarm(CardDto alarm);
    public delegate void OnRefreshAlarms(int idAlarm);
    public delegate void OnLoadAlarms();

    public class AlarmViewModel : ReactiveObject, IRoutableViewModel, IDisposable
    {
        private string _viewTitle;
        private readonly IAlarmService _alarmService = Locator.Current.GetService<IAlarmService>();
        private readonly IAlarmWorkflowService _alarmWorkflowService = Locator.Current.GetService<IAlarmWorkflowService>();
        private readonly IUserService _userService = Locator.Current.GetService<IUserService>();
        private readonly IAppContainer _container = Locator.Current.GetService<IAppContainer>();
        private readonly ICatalogService _catalogService = Locator.Current.GetService<ICatalogService>();
        public int _dvfid = -1;
        private List<AlarmDTO> _listAlarm = new List<AlarmDTO>();
        private Guid _parent = Guid.Empty;
        private CatalogDTO _catalog = null;
        //private CardDto _newAlarm = null;
        private CardDto _cardAlarm;
        //private int _idAlarm;
        private readonly Configuration _config;
        //private bool _loaded = false;

        //  public OnNewAlarm OnNewAlarm;
        public OnRefreshAlarms OnRefreshAlarms;
        public OnLoadAlarms OnLoadAlarms;
        private bool _useMultipleIds = false;
        public List<int> _dvfIds = new List<int>();
        public bool UseMultipleIds
        {
            get => _useMultipleIds;
            set => this.RaiseAndSetIfChanged(ref _useMultipleIds, value);
        }

        public Guid Parent
        {
            get => this._parent;
            set => this.RaiseAndSetIfChanged(ref this._parent, value);
        }

        public MainViewModel MainView { get; set; } = null;

        public CatalogDTO Catalog
        {
            get
            {
                return _catalog;
            }
            set
            {
                _catalogService.CurrentCatalog = value;
                this.RaiseAndSetIfChanged(ref _catalog, value); // Solo si necesitas disparar eventos UI
            }
        }

        public async Task<CatalogDTO> GetCatalogAsync()
        {
            if (_useMultipleIds && _dvfIds != null && _dvfIds.Count > 0)
            {
                _catalog = await _catalogService.GetValueCatalog(MainView.UserToken, _dvfIds);
            }
            else
            {
                _catalog = await _catalogService.GetValueCatalog(MainView.UserToken, _dvfid);
            }

            return _catalog;
        }

        public string ViewTitle
        {
            get => _viewTitle;
            set => this.RaiseAndSetIfChanged(ref _viewTitle, value);
        }

        public List<AlarmDTO> ListAlarm
        {
            get => _listAlarm;
            set => this.RaiseAndSetIfChanged(ref _listAlarm, value);
        }

        public CardDto CardAlarm
        {
            get => _cardAlarm;
            set => this.RaiseAndSetIfChanged(ref _cardAlarm, value);
        }

        //public CardDto NewAlarm
        //{
        //    get => _newAlarm;
        //    set => this.RaiseAndSetIfChanged(ref _newAlarm, value);
        //}

        //public int RefreshAlarms
        //{
        //    get => _idAlarm;
        //    set => this.RaiseAndSetIfChanged(ref _idAlarm, value);
        //}

        public IScreen HostScreen { get; protected set; }
        public string UrlPathSegment { get; protected set; }
        public RoutingState Router { get; }

        //private int _loadAlarms;
        //public int LoadAlarms
        //{
        //    get => _loadAlarms;
        //    set => this.RaiseAndSetIfChanged(ref _loadAlarms, value);
        //}

        // private readonly VmonitoringManagerConnection Signal = Locator.Current.GetService<VmonitoringManagerConnection>();

        public AlarmViewModel(MainViewModel mainView)
        {
            //var signal = Signal;
            this.MainView = mainView;
            //this.mainView.Signal.AlarmEventAction += AlarmEvent;
            this.MainView.Signal.RefreshAlarmsEventAction += RefreshAlarmsEventAction;
            this.MainView.Signal.OnClosed += Signal_OnClosed;
            this.MainView.Signal.OnReconected += Signal_OnReconected;

            _config = SmartClientEnvironmentUtils.GetConfiguration();

        }

        private void Signal_OnReconected(bool connected)
        {
            if (connected)
            {
                this.OnLoadAlarms?.Invoke();
            }
        }

        private void Signal_OnClosed(bool disconnected)
        {
            if (!disconnected)
            {
                this.OnLoadAlarms?.Invoke();
            }
        }

        public void Dispose()
        {
            // this.mainView.Signal.AlarmEventAction -= AlarmEvent;
            this.MainView.Signal.RefreshAlarmsEventAction -= RefreshAlarmsEventAction;
            this.MainView.Signal.OnClosed -= Signal_OnClosed;
            this.MainView.Signal.OnReconected -= Signal_OnReconected;
        }

        //private void AlarmEvent(dynamic d)
        //{
        //    if (d != null)
        //    {
        //        var t = JsonConvert.DeserializeObject<CardDto>(d.ToString());
        //        //Threads.RunInOtherThread(new Action[]
        //        //{
        //        //    () =>
        //        //    {
        //        //        if (AddNewAlarm(t))
        //        //        {
        //        //            Logger.Log("AlarmEvent Alarm", LogPriority.Information);
        //        //            NewAlarm = t;
        //        //        }
        //        //    }
        //        //}, null);

        //        if (AddNewAlarm(t))
        //        {
        //            Logger.Log("AlarmEvent Alarm", LogPriority.Information);
        //            this.OnNewAlarm?.Invoke(t);
        //        }

        //    }
        //}

        public async Task<List<CardDto>> GetData(FilterDTO filter, bool mapMode = false)
        {
            return await this._alarmService.Get(filter, MainView.UserToken, mapMode);
        }

        public async Task<string> GetSnapshot(int idAlarm)
        {   
            try
            {
                Logger.Log("GetSnapshot Entered AlarmId " + idAlarm, LogPriority.Information);
                string result = await this._alarmService.GetSnapshot(MainView.UserToken, idAlarm);
                Logger.Log("GetSnapshot Leaved AlarmId " + idAlarm, LogPriority.Information);
                return result;
            }
            catch (Exception ex)
            {
                Logger.Log(" GetSnapshot AlarmId " + idAlarm + " Exception " + ex.Message, LogPriority.Fatal);
                //Logger.Log(ex);
                throw ex;
            }
        }
        public async Task<Dictionary<int, string>> GetSnapshot(List<int> idAlarms)
        {
            try
            {
                Logger.Log("GetSnapshots Entered AlarmId ", LogPriority.Information);
                var result = new Dictionary<int, string>();
                bool alarmSnapshot = ((_config.AppSettings.Settings["AlarmSnapshot"] != null) && bool.Parse(_config.AppSettings.Settings["AlarmSnapshot"].Value));
                if (alarmSnapshot)
                {
                    result = await this._alarmService.GetSnapshot(MainView.UserToken, idAlarms);
                }
                else
                {
                    foreach (int idAlarm in idAlarms)
                    {
                        result.Add(idAlarm, "");
                    }
                }

                Logger.Log("GetSnapshot Leaved AlarmId ", LogPriority.Information);
                return result;
            }
            catch (Exception ex)
            {
                Logger.Log(" GetSnapshots Exception " + ex.Message, LogPriority.Fatal);
                //Logger.Log(ex);
                throw ex;
            }
        }

        public async Task<bool> DiscardAllAlarms(DiscardAllAlarms obj)
        {
            try
            {
                return await this._alarmService.DiscardAllAlarms(MainView.UserToken, obj);
            }
            catch (Exception ex)
            {
                Logger.Log(" DiscardAllAlarms Exception " + ex.Message, LogPriority.Fatal);
                //Logger.Log(ex);
                return false;
            }
        }

        public Task<AlarmDTO> GetAlarm(int idAlarm)
        {
            return this._alarmService.Get(MainView.UserToken, idAlarm);
        }

        public async Task<List<string>> GetTypesAlarms()
        {
            return await this._alarmService.GetTypesAlarms(MainView.UserToken);
        }

        public void Attend(AlarmDTO alarm)
        {
            alarm.AttendedBy = (int)MainView.User.UserId;
            alarm.Level = alarm.Level != AlarmLevels.CriticalChecked ? alarm.Level : AlarmLevels.CriticalChecked;
            this._alarmService.Attend(MainView.UserToken, alarm);
        }

        //public void Discard(AlarmDTO alarm)
        public async Task<CardDto> Discard(AlarmDTO alarm)
        {
            alarm.AttendedBy = (int)MainView.User.UserId;
            alarm.Level = AlarmLevels.CriticalDiscard;
            return await this._alarmService.Discard(MainView.UserToken, alarm);
        }
        public async Task<AlarmWorkflowDTO> GetAlarmWorkflow(int idAlarmWorkflow)
        {
            return await this._alarmWorkflowService.Get(MainView.UserToken, idAlarmWorkflow);
        }

        //public async Task<List<EmailNotifyDTO>> GetEmailNotify()
        //{
        //    List<EmailNotifyDTO> list = await this._userService.GetUserNotify(MainView.UserToken);
        //    if (list == null)
        //    {
        //        list = new List<EmailNotifyDTO>();
        //    }
        //    return list;

        //}

        public async Task<CameraDTO> GetCamera(int elementId)
        {
            return await Vmon5Service.GetCamera(elementId, MainView.UserToken);
        }

        public async Task<List<CatalogSite>> GetSite()
        {
            if (this.Catalog == null)
            {
                this.Catalog = new CatalogDTO();
                this.Catalog = await Vmon5Service.GetSiteInfo(MainView.UserToken);
            }
            return this.Catalog.Sites;
        }

        private bool AddNewAlarm(CardDto t)
        {
            if (t.DeviceType == "VIN"
                || t.DeviceType == "DI")
            {
                var ObjectsId = MainView.Catalog.Sites.SelectMany(y => y.Cameras.Select(z => z.ObjectId)).ToList();

                return ObjectsId.Exists(x => x == t.ObjectId);
            }
            else
            {
                return true;
            }
        }

        public void SetName(Apps app)
        {
            if (_container.AppsActives.ContainsKey(Parent) == false)
            {
                _container.AppsActives.Add(Parent, app);
            }

            var n = _container.AppsActives.Where(x => x.Value == app).Count();
            MainView.ApplicationTitle = n > 1 ? $"{Resources.WelcomeAlarm} {n}" : Resources.WelcomeAlarm;
        }

        public void SendNotify(UsersNotifyAlarmDTO request)
        {
            this._userService.SendEmailNotify(request, this.MainView.UserToken);
        }

        public async Task<DeviceFeatureDTO> GetDeviceFeature(int deviceFeatureId)
        {
            return await this._alarmService.GetDeviceFeatures(MainView.UserToken, deviceFeatureId);
        }

        private void RefreshAlarmsEventAction(dynamic idAlarm)
        {
            //if (idAlarm > 0)
            //{
            this.OnRefreshAlarms?.Invoke(Convert.ToInt32(idAlarm));
            //}
        }

        public async Task<List<CardDto>> GetDataHistory(FilterDTO filter, bool mapMode = false)
        {
            return await this._alarmService.GetHistory(filter, MainView.UserToken, mapMode);
        }

        public void RetrieveAlarm(AlarmDTO alarm)
        {
            alarm.AttendedBy = (int)MainView.User.UserId;
            alarm.Level = AlarmLevels.CriticalWoSound;
            this._alarmService.Discard(MainView.UserToken, alarm);
        }

        public async Task<List<OptionObjectDTO>> GetSitesName(int page, int take, int typeElement, int userId, GroupType typeGroup, string filter, string search = "")
        {
            var list = new List<OptionObjectDTO>();
            var _sites = await Vmon5Service.GetSiteCatalog(MainView.UserToken, page, take, typeElement, userId, typeGroup, filter, search);


            if (_sites != null)
            {
                foreach (var item in _sites.Elements)
                {
                    List<OptionObjectDTO> listSite = new List<OptionObjectDTO>
                    {
                     new OptionObjectDTO()
                     {
                         Name = item.Name,
                         Key = item.Id.ToString(),
                         Tag = item.Id,
                         Item = item,
                         count = _sites.Count,
                     }
                    };
                    list.Add(listSite);
                }

            }
            return list;
        }
    }
}
