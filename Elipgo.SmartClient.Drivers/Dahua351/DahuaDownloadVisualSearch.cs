...
namespace Elipgo.SmartClient.Drivers.Dahua351
{
    public class DahuaDownloadVisualSearch : IDriverDownloadVisualSearch, IDisposable, IConectionNotification
    {
        ...
        private void InitializeDriver()
        {
            ...
            {
                if (loginControl.GetDeviceLogin(Camera, out loginHandle, Common.Enum.Drivers.NETSDK_351, this))
                {
                    loginControl.AddChannel(Camera, this, Common.Enum.Drivers.NETSDK_351);
                ...
                var videoPort = Camera.VideoPort == 0 ? (ushort)37777 : (ushort)Camera.VideoPort;
                while (_try <= (tryLimit + 1))
                {
                    ...
                    if (loginHandle == IntPtr.Zero)
                    {
                        ...
                        {
                            loginControl.AddDevice(Camera, (IntPtr)loginHandle, this, Common.Enum.Drivers.NETSDK_351);
        ...
        public void Start(string fileName, DateTime startTime, DateTime endTime)
        {
            ...
            Logger.Log(String.Format("Dahua 351 start entered camera {0} Channel{1} start: {2} end:{3} filaname:{4} ", Camera.Name, Camera.Channel - 1, startTime, endTime, fileName), LogPriority.Information);
            InitializeDriver();
            ...
            while (_try <= (tryLimit + 1)) // prevent indefinite cycle
            {
                ...
                try
                {
                    ...
                    {
                        Logger.Log(String.Format(" DahuaDownload start failed channel{0} start: {1} end:{2} filaname:{3} loginHandle:{4} timeDownLoadPosHandle:{5}", Camera.Channel - 1, StartTime, EndTime, FileName, loginHandle, timeDownLoadPosHandle), LogPriority.Information);
        ...
        private void TimeDownLoadPosCallBack(IntPtr lPlayHandle, uint dwTotalSize, uint dwDownLoadSize, int index, NET_RECORDFILE_INFO recordfileinfo, IntPtr dwUser)
        {
            ...
            if ((uint)downLoadOver != dwDownLoadSize)
            {
                ...
                Logger.Log(string.Format("Device {0} is downloading a file, progress {1}", Camera.Name, progress), LogPriority.Information);
