...
namespace Elipgo.SmartClient.Drivers.Dahua351
{
    public partial class DahuaInstantPlaybackUserControl : UserControl, IDriverInstantPlayback, IDisposable, IConectionNotification
    {
        ...
        private int TimeReConnectionCheck;
        private RecorderDTOSmall Recorder;
        private string nameTab = string.Empty;
        private EnumDrivers driver;
        private bool _isBusyRew = false;
        private bool _isBusyFwd = false;

        public DahuaInstantPlaybackUserControl(CameraDTO camera, Profile profile, bool hideControls, DateTime selectedDateTime, RecorderDTOSmall recorder, string nameTab, EnumDrivers driver, AbsNetSDK _UseDriver, bool isDiagnostic = false, DateTime? selectedEndDateTime = null)
        {
            InitializeComponent();
            this.driver = driver;
            //this.UseDriver = _UseDriver;
            Task.Run(() =>
            {
        ...
        private void DahuaInstantPlaybackUserControl_Resize(object sender, EventArgs e)
        {
            if (Screen.AllScreens.Any(x => x.WorkingArea.Contains(Cursor.Position)))
            {
                ...
                if (instantPlayerView != null && instantPlayerView.WindowState == FormWindowState.Maximized)
                {
                    ...
                    {
                        this.PanelVideo.Height = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.7480M), 2)) + 35;
                        slider.Location = new Point(0, 550);
                    }
                    if (!IsPlaying)
                    {
                        Jump(1, false);
                ...
                else if (instantPlayerView != null && instantPlayerView.WindowState == FormWindowState.Normal)
                {
                    ...
                    else if (main.Width == 1024 && main.Height == 768)
                    {
                        ...
                        sliderY = Convert.ToInt32(Math.Round(Convert.ToDecimal(this.Height * 0.8166M), 2));
                    }
                    else
                    {
                ...
                {
                    ButtonFullScreen.Location = new Point(ButtonFullScreenX + 20, ButtonFullScreenY);
                    ButtonBookmark.Location = new Point(ButtonBookmarkX + 20, ButtonBookmarkY);
                    ButtonZoom.Location = new Point(ButtonZoomX + 20, ButtonZoomY);
                    ButtonSnapshot.Location = new Point(ButtonSnapshotX + 20, ButtonSnapshotY);
                    ButtonVideoclip.Location = new Point(ButtonVideoclipX + 20, ButtonVideoclipY);
                    ButtonListen.Location = new Point(ButtonListenX + 20, ButtonListenY);
                }
            }
        }
            ...
            try
            {
                if (!this.IsDisposed)
                {
                    ...
                    if (PlayBackByTime() == false)
                    {
                        ...
                        Task.Delay(1000 * (tryCount + TimeReConnectionCheck)).ContinueWith(t => TryToReConnect());
                    }
                    else
                    {
            ...
            try
            {
                switch (connectionState)
                {
                    ...
                    case PlaybackConnectionState.NoRecording:
                        this.PanelVideo.SendToBack();
                        this.picture.Size = PanelVideo.Size;
        ...
        private void Slider_ValueChanged(object sender, EventArgs e)
        {
            ...
            //NO habilitar esta line revisar con Roberto el porque
        ...
        private void CustomDispose(bool removeControls = true)
        {
            try
            {
                if (!string.IsNullOrEmpty(nameTab))
                {
                    NETClient.SetDriver(driver);
                    if (loginControl.RemoveChannelAndCanLogout(Camera, this, driver))
                    {
                        Logger.Log(String.Format(" Dispose Logout Dahua {4} {0} {1} {2} {3}", Camera.Name, Camera.Host, Camera.VideoPort.ToString(), Camera.User, driver.ToString()), LogPriority.Information);
                        ...
                        {
                            this.Controls.Clear();
                            this.OnDispose?.Invoke(driver, nameTab, this);
                        }
            ...
            {
                Logger.Log(String.Format("Error al ejecutar el driver {1} {0}" + ex.StackTrace, driver.ToString()));
                notification.Show("Error al ejecutar el metodo playback custome dispose", null);
            }
        }
        ...
        private void ReConnectCallBack(IntPtr lLoginID, IntPtr pchDVRIP, int nDVRPort, IntPtr dwUser)
        {
            ...
            {//if 
                ...
                SetVisivility(PlaybackConnectionState.Connected);
                loginControl.Connect(lLoginID, this.playbackHandle);
        ...
        public bool Play()
        {
            ...
            {
                Logger.Log(String.Format("isPlay Exception {0} {1} {2} {3} ", Camera.Name, Camera.Host, Camera.VideoPort.ToString(), Camera.User) + ex.Message, LogPriority.Fatal);
                notification.Show(string.Format("{0} - {1}", Camera.Name, ex.Message), null);
                IsPlaying = false;
            }
            return IsPlaying;
        ...
        private void IsPlay()
        {
            try
            {
                ...
                if (Login())
                {
                    ...
                    Logger.Log(String.Format(" Play Dahua {4} entered  {0} {1} {2} {3} ", Camera.Name, Camera.Host, Camera.VideoPort.ToString(), Camera.User, driver.ToString()), LogPriority.Information);
                    ...
                    if (IsPlaying)
                    {
                        ...
                        Logger.Log(String.Format("Play Dahua {4} Connected  {0} {1} {2} {3} ", Camera.Name, Camera.Host, Camera.VideoPort.ToString(), Camera.User, driver.ToString()), LogPriority.Information);
                        SetVisivility(PlaybackConnectionState.Connected);
                    }
                    else
                    {
                        if (!this.IsDisposed)
                        {
                            ...
                            {
                                SetVisivility(PlaybackConnectionState.Reconnecting);
                                m_PlayBack_StartTime = m_PlayBack_StartTime.AddMinutes(-30);
                                Task.Delay(1000 * (retryCount + TimeReConnectionCheck)).ContinueWith(p => IsPlay());
                                Logger.Log(String.Format("Dahua Play() Error to Camera play Connected {4} of {5}:  {0} {1} {2} {3} ", Camera.Name, Camera.Host, Camera.VideoPort.ToString(), Camera.User, retryCount, retryLimit), LogPriority.Information);
                                retryCount++;
                            }
                            else
                            {
                ...
                else
                {
                    ...
                    {
                        SetVisivility(PlaybackConnectionState.Reconnecting);
                        Threads.RunInOtherThread(new Action[] { () => System.Threading.Thread.Sleep(2000 * retryCount) }, () => IsPlay());
        ...
        private bool PlayBackByTime()
        {
            ...
            retry:
                ...
                if (NETClient.GetLastError() == "Handle is invalid")
                {
                    Logger.Log("Device PlayBackByTime Handle is invalid --> loginHandle " + loginHandle + " Error: " + NETClient.GetLastError(), LogPriority.Information);
                }
                else if (playbackHandle == IntPtr.Zero)
                {
                ...
                switch (playbackRetry)
                {
                    case PlaybackRetry.StartRetry:
                        ...
                        else if (playbackHandle == IntPtr.Zero && NETClient.GetLastError() == "Failed to get connect session information")
                        {
            ...
            {
                Logger.Log($"PlayBackByTime Exception: {ex.Message} ", LogPriority.Fatal);
                throw;
            }
        }
        ...
        private bool Login()
        {
            if (loginControl.GetDeviceLogin(Camera, out loginHandle, driver, this))
            {
                loginControl.AddChannel(Camera, this, driver);
            ...
            try
            {
                ...
                else
                {
                    if (!this.IsDisposed)
                    {
                        loginControl.AddDevice(Camera, loginHandle, this, driver);
        ...
        public bool Snapshot(string path)
        {
            ...
            {
                NETClient.SetDriver(driver);
                var result = NETClient.CapturePicture(playbackHandle, path, EM_NET_CAPTURE_FORMATS.JPEG);
                return result;
            }
            catch (Exception ex)
            {
        ...
        {
            if (dwDownLoadSize >= uint.MaxValue)
            {
                m_DispatcherTimer.Stop();
                Stop();
                IsPlaying = false;
                SetStartDateTime(actualTime);
                Play();
            }
        }
        ...
        public bool PlayNoAsync()
        {
            ...
            {
                Logger.Log(String.Format("PlayNoAsync Exception: {0} {1} {2} {3} ", Camera.Name, Camera.Host, Camera.VideoPort.ToString(), Camera.User) + ex.Message, LogPriority.Fatal);
                return false;
            }
        }
