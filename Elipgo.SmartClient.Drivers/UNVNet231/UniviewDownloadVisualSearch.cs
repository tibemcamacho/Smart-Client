...
{
    public class UniviewDownloadVisualSearch : IDriverDownloadVisualSearch, IDisposable, IConectionNotification
    {
        ...
        public UniviewDownloadVisualSearch(CameraDTO camera)
        {
            Camera = camera;
        ...
        public void Start(string fileName, DateTime startTime, DateTime endTime)
        {
            ...
            while (_try <= (tryLimit + 1)) // prevent indefinite cycle
            {
                try
                {
                    ...
                    {
                        Logger.Log(string.Format("Device Start  {0} filehandle is Zero, Error: {1} ", Camera.Name, NETDEVSDK.NETDEV_GetLastError()), LogPriority.Fatal);
                        if (TryLimit(ref _try, string.Format("Device Start  {0} filehandle is Zero, Error: {1} ", Camera.Name, NETDEVSDK.NETDEV_GetLastError())))
                        {
                    ...
                    else
                    {
                        ...
                        timerDownload.Elapsed += new System.Timers.ElapsedEventHandler(OnTimedEvent);
        ...
        {
            Logger.Log(string.Format("Device {0} is downloading a file, progress {1}", Camera.Name, progressValue), LogPriority.Information);
