    ...
    public partial class HikvisionLiveUserControl : UserControl, IDriverLive, IDisposable, IConectionNotification
    {
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            if (DigitalZoomStatus)
            {
                ...
                int picRelX = relativePosition.X - _picture.Location.X;
                int picRelY = relativePosition.Y - _picture.Location.Y;
            ...
            try
            {
                ...
                if (DigitalZoomStatus)
                {
                    if (_picture.Location.X < 0)
                    {
                        ...
                        int contentX = mouse.X - _picture.Location.X;
                        int contentY = mouse.Y - _picture.Location.Y;
                        ...
                        newX = Math.Min(0, Math.Max(this.Width - _picture.Width, newX));
                        newY = Math.Min(0, Math.Max(this.Height - _picture.Height, newY));

                        _picture.Location = new Point(newX, newY);
                    }
        ...
        private void ZoomPicture(bool direction, int eventPositionX, int eventPositionY)
        {
            ...
            int parentX = eventPositionX + _picture.Location.X;
            int parentY = eventPositionY + _picture.Location.Y;

            // Calculate content fraction under cursor using current picture dimensions
            double fracX = (double)eventPositionX / _picture.Width;
            double fracY = (double)eventPositionY / _picture.Height;
            ...
            try
            {
                switch (connectionState)
                {
                    case PlaybackConnectionState.Disconnected:
                        this._panelConnection.BringToFront();
