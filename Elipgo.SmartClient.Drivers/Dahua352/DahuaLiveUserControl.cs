...
namespace Elipgo.SmartClient.Drivers.Dahua352
{
    ...
    public partial class DahuaLiveUserControl : UserControl, IDriverLive, IDisposable, IConectionNotification
    {
        ...
        public DahuaLiveUserControl(CameraDTO camera, Profile profile, bool initAudio)
        {
            InitializeComponent();
            Camera = camera;
        ...
        {
            Logger.Log($"Dispose Dahua  {Camera.Name} {Camera.Host} {Camera.VideoPort} {Camera.User}", LogPriority.Information);
            ...
            Logger.Log($"Dispose Logout Dahua {Camera.Name} {Camera.Host} {Camera.VideoPort} {Camera.User}", LogPriority.Information);
        ...
        private void DahuaLiveUserControl_Click(object sender, EventArgs e)
        {
            ...
            if (DigitalZoomStatus)
            {
                if (picture.Location.X < 0)
                {
                    ...
                    Point relativePoint = new Point(100 * mousePosition.X / this.Width, 100 * mousePosition.Y / this.Height);
                    Point pictureOffset = new Point(picture.Width * relativePoint.X / 100, picture.Height * relativePoint.Y / 100);
                    picture.Location = new Point(mousePosition.X - pictureOffset.X, mousePosition.Y - pictureOffset.Y);
                }
        ...
        public async Task<bool> Snapshot(string path, string token, int id)
        {
            ...
            try
            {
                ...
                bool captureSuccessful = NETClient.SnapPictureEx(DeviceLogin, snapshotParams, IntPtr.Zero); //call capture function.
                if (captureSuccessful)
                {
                    if (!File.Exists(_snapshotPath))
                    {
                        var imagen64 = await Vmon5Service.Snapshot(id, token);
                        if (imagen64 != ImageHelper.ImageToBase64Default())
                        {
                        ...
                        {
                            bool result = NETClient.CapturePicture(this._realHandle, _snapshotPath, EM_NET_CAPTURE_FORMATS.JPEG);
                            if (!result)
                            {
                                _notification.Show(NETClient.GetLastError(), null);
                                return false;
                            }
                        }
                    }
        ...
        public bool VideoClipStart(string path)
        {
            try
            {
                ...
                bool ret = NETClient.SaveRealData(_realHandle, path);
                if (ret)
                {
                    _saveRealDataDictionary.Add(_realHandle, path);
                }
                ClipStatus = ret;
                return ret;
            }
        ...
        public bool VideoClipStop()
        {
            try
            {
                bool ret = NETClient.StopSaveRealData(_realHandle);
                if (ret)
                {
        ...
        {
            Logger.Log($"Disconect Dahua 352 disconnected {Camera.Name} {Camera.Host} {Camera.VideoPort} {Camera.User}", LogPriority.Information);
            SetVisivility(PlaybackConnectionState.Reconnecting);
            NETClient.StopRealPlay(_realHandle);
        }
        ...
        private bool Login()
        {
            //if (loginControl.GetDeviceLogin(Camera, out DeviceLogin, Common.Enum.Drivers.NETSDK_352))
            //{
            ...
            if (true)
            {
                ...
                try
                {
                    ...
                    if (DeviceLogin != IntPtr.Zero)
                    {
                        //loginControl.AddDevice(Camera, DeviceLogin, this, Common.Enum.Drivers.NETSDK_352);
        ...
        private void ZoomPicture(bool direction, int eventPositionX, int eventPositionY)
        {
            ...
            Point relativePoint = new Point((100 * eventPositionX) / this.Width, (100 * eventPositionY) / this.Height);
            Point pictureOffset = new Point((pictureSize.Width * relativePoint.X) / 100, (pictureSize.Height * relativePoint.Y) / 100);
            Point pictureLocation = new Point((eventPositionX - pictureOffset.X), (eventPositionY - pictureOffset.Y));
