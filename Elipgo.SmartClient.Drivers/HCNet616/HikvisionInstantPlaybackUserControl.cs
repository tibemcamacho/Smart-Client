...
namespace Elipgo.SmartClient.Drivers.HCNet616
{
    public partial class HikvisionInstantPlaybackUserControl : UserControl, IDriverInstantPlayback, IDisposable, IConectionNotification
    {
        ...
        public HikvisionInstantPlaybackUserControl(CameraDTO camera, Profile profile, bool hideControls, DateTime selectedDateTime, RecorderDTOSmall recorder, bool isDiagnostic = false, DateTime? selectedEndDateTime = null)
        {
            InitializeComponent();
        ...
        private void DahuaInstantPlaybackUserControl_Resize(object sender, EventArgs e)
        {
            if (Screen.AllScreens.Length >= 1 && Screen.AllScreens.Any(x => x.WorkingArea.Contains(Cursor.Position)))
            {
                ...
                if (instantPlayerView != null && instantPlayerView.WindowState == FormWindowState.Maximized)
                {
                    ...
                    {
                        this.PanelVideo.Height = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.6666M), 2)) + 35;
                ...
                else if (instantPlayerView != null && instantPlayerView.WindowState == FormWindowState.Normal)
                {
                    ...
                    if (main.Width == 1024 && main.Height == 768)
                    {
                        ...
                        slider.Location = new Point(0, 296);
                    }
                    else if (main.Width < 1400)
                    {
                        panelVideoHeigth = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.2604M), 2));
                        var sliderY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.2460M), 2));
                ...
                var btnLocation = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019M), 2));
                ...
                var buttonPauseY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var buttonSlowY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonFastY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonRewSecsY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonFwdSecsY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonZoomY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonFullScreenY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonBookmarkY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonSnapshotY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonListenY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var ButtonVideoclipY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019), 2));
                ...
                var labelSpeedHeight = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019M), 2));

                var LabelSpeedX = Convert.ToInt32(Math.Round(Convert.ToDecimal(this.Width * 0.347), 2));
                var LabelSpeedY = Convert.ToInt32(Math.Round(Convert.ToDecimal(main.Height * 0.019M), 2));
        ...
        private bool OpenCamera(int channel)
        {
            ...
            try
            {
                ...
                struVodPara.struIDInfo.dwChannel = (uint)HikvisionHelper.GetChannelNumber(channel, this.StartDigitalChannel); //Channel number  
                struVodPara.hWnd = GetHandle();//handle of playback

                //if (!_isVault)
                //{
                //    m_PlayBack_StartTime = m_PlayBack_StartTime.AddSeconds(-2);
                //}
        ...
        private IntPtr GetHandle()
        {
            ...
            {
                return (IntPtr)picture.Invoke(new Func<IntPtr>(() => picture.Handle));
            }
            ...
            if (res)
            {
                if (Login())
                {
                    if (instantPlayback)
                    {
                        ...
                        SetSliderValueSafe((int)(m_PlayBack_EndTime - m_PlayBack_StartTime).TotalSeconds);
                    }
                    return Play();
                }
                else
        ...
        private void SetSliderValueSafe(int value)
        {
            ...
            if (slider.InvokeRequired)
            {
                ...
                {
                    if (!this.IsDisposed && !slider.IsDisposed)
                        slider.Value = value;
                }));
            }
            else
        ...
        private void SetVisivility(PlaybackConnectionState connectionState)
        {
            ...
            if (this.picture.InvokeRequired)
            {
                ...
                this.BeginInvoke(d, new object[] { connectionState });
                return; // Importante: salir después de invocar
            }
            try
            {
        ...
        public bool VideoClipStart(string path)
        {
            try
            {
                CHCNetSDK.NET_DVR_MakeKeyFrame(loginHandle, HikvisionHelper.GetChannelNumber(Camera.Channel, this.StartDigitalChannel));
